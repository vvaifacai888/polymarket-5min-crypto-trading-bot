"""
run_bot.py  --  Unified entry point for the Polymarket BTC 15-min bot

Usage:
    python run_bot.py --test-mode        # simulation, trade every minute, full tracking
    python run_bot.py --simulation       # simulation, normal 15-min clock
    python run_bot.py --live             # REAL MONEY - requires .env keys
...
Test mode features:
    ...
    - Run view_paper_trades.py at any time in a separate window to see results
"""
...
# ── Configure loguru for test mode (cleaner output) ──────────────────────────
def setup_logging(test_mode: bool, verbose: bool):
    logger.remove()
    fmt = (
...
def preflight(simulation: bool) -> bool:
    """Check required env vars and dependencies before starting."""
    ok = True
    print()
    print("=" * 70)
    ...
    required_live = ["POLYMARKET_PK", "POLYMARKET_API_KEY",
...
# ── Test-mode banner printed after every trade ────────────────────────────────
def print_live_dashboard(paper_trades_path: str = "paper_trades.json"):
    ...
    print("+" + "-" * 50 + "+")
    print(f"|  Run:  python view_paper_trades.py  for full detail   |")
    print("+" + "-" * 50 + "+")
    print()
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python run_bot.py --test-mode          # fast simulation (1-min intervals)
  python run_bot.py --simulation         # normal simulation (15-min intervals)
  python run_bot.py --live               # REAL MONEY
  python run_bot.py --test-mode --verbose  # with DEBUG logs
  python run_bot.py --test-mode --no-grafana
        """
    )
    ...
    mode_group.add_argument(
        ...
        help="Simulation mode with 1-min trade clock and 5-min learning cycle (fastest way to test)"
    )
    mode_group.add_argument(
        "--simulation", action="store_true",
        help="Simulation mode with normal 15-min clock"
    )
    mode_group.add_argument(
        "--live", action="store_true",
        help="LIVE mode — REAL MONEY at risk. Requires all API keys in .env"
    )
    ...
    if test_mode:
        ...
        print("         View results:  python view_paper_trades.py")
    ...
    # ── Import and run bot ────────────────────────────────────────────────────
    try:
        from bot import run_integrated_bot
    except ImportError as e:
        logger.error(f"Could not import bot.py: {e}")
    ...
    try:
        ...
        print("\nShutting down...")
        print("Trades saved to paper_trades.json")
        print("Run:  python view_paper_trades.py  to see results")
