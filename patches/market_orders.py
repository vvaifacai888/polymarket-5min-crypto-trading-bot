...
=====================
Monkey-patch for ``PolymarketExecutionClient._submit_market_order``.
...
The standard Nautilus adapter requires ``quote_quantity=True`` for BUY market
orders, but our strategy sends token quantities (``quote_quantity=False``).

This patch intercepts BUY market orders and forces them to use the configured
USD amount from the ``MARKET_BUY_USD`` environment variable (default $1).
The minimum order is $1 USD for market BUY orders, NOT 5 tokens.
"""
...
def apply_market_order_patch() -> bool:
    """Apply monkey-patch to ``PolymarketExecutionClient``.  Idempotent."""
    ...
    try:
        ...
        async def _patched_submit_market_order(self, command, instrument):
            ...
            if order.side == OrderSide.BUY:
                ...
                # SELL: use token quantity (base-denominated)
        ...
        logger.error(f"Failed to import required modules for market_orders patch: {e}")
        return False
    except Exception as e:
        logger.error(f"Failed to apply market_orders patch: {e}")
        import traceback
        traceback.print_exc()
        return False
